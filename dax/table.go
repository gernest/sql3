...
func (opts *DatabaseOptions) Set(option string, value string) error {
	...
	case DatabaseOptionWorkersMin:
		...
		if err != nil {
			return fmt.Errorf("converting value to int: %s %w", value, err)
		}
		opts.WorkersMin = min
		...
		opts.WorkersMax = min
	default:
		return fmt.Errorf("unsupported database option: %s", option)
	}
...
func (t *Table) CreateID() (TableID, error) {
	if t.ID != "" {
		return "", fmt.Errorf("CreateID called on table %+v that already has ID", t)
	}
	...
	if _, err := rand.Read(rn); err != nil {
		return "", fmt.Errorf("getting random data %w", err)
	}
...
func QualifiedTableIDFromKey(key string) (QualifiedTableID, error) {
	...
	default:
		return QualifiedTableID{}, fmt.Errorf("invalid key: %s", key)
	}
}
...
func QualifiedDatabaseIDFromKey(key string) (QualifiedDatabaseID, error) {
	...
	default:
		return QualifiedDatabaseID{}, fmt.Errorf("invalid key: %s", key)
	}
}
...
func SplitFieldType(s string) (BaseType, []interface{}, error) {
	...
	if len(parts) > 1 {
		...
		if len(parenParts) != 2 {
			return "", nil, fmt.Errorf("invalid type qualifier: %s", s)
		}
		paren = parenParts[0]
	}
	...
	case BaseTypeDecimal:
		...
		if err != nil {
			return "", nil, fmt.Errorf("parsing int from string: %s %w", paren, err)
		}
...
func (f *Field) constraints() string {
	...
	switch f.Type {
	case BaseTypeInt:
		sql += fmt.Sprintf(" MIN %d MAX %d", f.Options.Min, f.Options.Max)
