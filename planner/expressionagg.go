...
func (m *aggregateSum) Update(ctx context.Context, row types.Row) error {
	...
	case *parser.DataTypeDecimal:
		...
		if m.sum != nil {
			dsum, ok = m.sum.(decimal.Decimal)
			if !ok {
				return sql3.NewErrInternalf("unexpected type conversion '%T'", m.sum)
			}
		} else {
			dsum = decimal.NewDecimal(0, dataType.Scale)
		}
		dsum = decimal.AddDecimal(dsum, val)
		m.sum = dsum
...
func (a *aggregateAvg) Update(ctx context.Context, row types.Row) error {
	...
	case *parser.DataTypeDecimal:
		...
		var ok bool
		var aggVal decimal.Decimal
		if a.sum == nil {
			aggVal = decimal.NewDecimal(0, returnType.Scale)
		} else {
			aggVal, ok = a.sum.(decimal.Decimal)
			if !ok {
				return sql3.NewErrInternalf("unexpected type conversion '%T'", v)
			}
		}
		...
		case *parser.DataTypeDecimal:
			thisVal, ok := v.(decimal.Decimal)
			if !ok {
				return sql3.NewErrInternalf("unexpected type conversion '%T'", v)
			}

			a.sum = decimal.AddDecimal(thisVal, aggVal)
		...
		case *parser.DataTypeInt, *parser.DataTypeID:
			...
			thisVal := decimal.FromInt64(thisIVal, returnType.Scale)
			a.sum = decimal.AddDecimal(thisVal, aggVal)
...
func (a *aggregateAvg) Eval(ctx context.Context) (interface{}, error) {
	...
	case *parser.DataTypeDecimal:
		...
		if a.rows == 0 {
			return decimal.NewDecimal(0, returnType.Scale), nil
		}
		count := decimal.FromInt64(a.rows, returnType.Scale)

		sum, ok := a.sum.(decimal.Decimal)
		if !ok {
			return nil, sql3.NewErrInternalf("unexpected type conversion '%T'", a.sum)
		}
		return decimal.DivideDecimal(sum, count), nil
...
func (m *aggregateMin) Update(ctx context.Context, row types.Row) error {
	...
	case *parser.DataTypeDecimal:
		...
		aggVal, ok := m.val.(decimal.Decimal)
		if !ok {
			return sql3.NewErrInternalf("unexpected type conversion '%T'", v)
		}
...
func (m *aggregateCorr) Eval(ctx context.Context) (interface{}, error) {
	...
	d, err := decimal.FromFloat64WithScale(corr, 6)
	if err != nil {
		return nil, err
	}
	return d, nil
...
func (m *aggregateVar) Eval(ctx context.Context) (interface{}, error) {
	...
	d, err := decimal.FromFloat64WithScale(variance, 6)
	if err != nil {
		return nil, err
	}
	return d, nil
